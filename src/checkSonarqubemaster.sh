...
DEVELOP_PROJECT=${DEVELOP_PROJECT:-'develop'}

develop_stats=$(curl -s "${SONAR_HOST_URL}api/measures/component?component=$GIT_NAME-${DEVELOP_PROJECT}&metricKeys=bugs,security_rating,sqale_rating,coverage,duplicated_lines_density,lines_to_cover")
...
if [ $(echo $develop_stats | grep -i "not found" | wc -l) -ne 0 ]; then 
	echo "> DID not find ${DEVELOP_PROJECT} project"
	echo $develop_stats
	exit
fi
...
echo "### Sonarqube ${DEVELOP_PROJECT}/master comparison results:"
echo ""
...
if [ "$bugs" -ne 0 ]; then
	echo "  **FAILURE** - there are $bugs bugs in the ${DEVELOP_PROJECT} branch ( needs to be 0 )"
	echo "  "
	echo "  Please check the sonarqube link and fix them: ${SONAR_HOST_URL}project/issues?resolved=false&types=BUG&inNewCodePeriod=false&id=$GIT_NAME-${DEVELOP_PROJECT}"
        exit_error=1
else
	echo "  OK ( no bugs )"
fi
...
if [ "$metrics_master" -lt "$metrics_develop" ]; then
        echo "  **FAILURE** - the security rating (1=A) is worse in the ${DEVELOP_PROJECT} branch ($metrics_develop) than the master ($metrics_master) branch"
        echo "  "
       	echo "  Please check the sonarqube link and fix this: ${SONAR_HOST_URL}project/issues?resolved=false&types=VULNERABILITY&inNewCodePeriod=false&id=$GIT_NAME-${DEVELOP_PROJECT}"
        exit_error=1
else
...
if [ "$metrics_master" -lt "$metrics_develop" ]; then
        echo "  **FAILURE** - the maintainability rating (1=A) is worse in the ${DEVELOP_PROJECT} branch ($metrics_develop) than the master ($metrics_master) branch "
        echo "  "
	echo "  Please check the sonarqube link and fix this: ${SONAR_HOST_URL}component_measures?id=$GIT_NAME-${DEVELOP_PROJECT}&metric=sqale_rating&view=list"
        exit_error=1
else
        echo "  OK ( $metrics_develop <= $metrics_master )"
fi
...
if [ "$metrics_master" -lt "$metrics_develop" ]; then
        ...
        echo "  **FAILURE** - the percentage of duplicated lines is bigger in the ${DEVELOP_PROJECT} branch ($metrics_develop) than the master ($metrics_master) branch"
        echo "  "
	echo "  Please check the sonarqube link and fix this: ${SONAR_HOST_URL}component_measures?id=$GIT_NAME-${DEVELOP_PROJECT}&metric=duplicated_lines_density&view=list"	
        exit_error=1
else
	metrics_develop=$(echo $metrics_develop | awk '{printf("%.2f",$1/100)}')
	metrics_master=$(echo $metrics_master | awk '{printf("%.2f",$1/100)}')
...
if [ "$metrics_master" -gt "$metrics_develop" ]; then
	...
	if [ "$metrics_master" -eq 10000 ]; then
	  ...
	  echo "  **FAILURE** - The percentage of coverage is smaller in the ${DEVELOP_PROJECT} branch ($metrics_develop) than the master ($metrics_master) branch"
          echo "  "
	  echo "  Please check the sonarqube link and fix this: ${SONAR_HOST_URL}component_measures?id=$GIT_NAME-${DEVELOP_PROJECT}&metric=coverage&view=list"
          exit_error=1
	fi
else
        metrics_develop=$(echo $metrics_develop | awk '{printf("%.2f",$1/100)}')
