...
if [ -z "$GIT_CHANGE_ID" ] && [[ "$GIT_BRANCH" == "master" ]] ; then
	...
	echo "Starting release on github"
	    ...
	    body=$(cat releasefile  | sed -E ':a;N;$!ba;s/\r{0,1}\n/\\n/g' | sed 's/"/\\\"/g' | sed 's/[R|r]efs #\([0-9]*\)/Refs \[#\1\]\(https:\/\/taskman.eionet.europa.eu\/issues\/\1\)/g' )
	    echo "{\"tag_name\": \"$version\",\"name\": \"$version\", \"target_commitish\":\"${GIT_BRANCH}\",  \"body\":  \"$body\"}"

	    curl   -X POST   -H "Accept: application/vnd.github.v3+json"  -H "Authorization: bearer $GITHUB_TOKEN" -d @-  https://api.github.com/repos/${GIT_ORG}/${GIT_NAME}/releases  << CURL_DATA
{"tag_name": "$version","name": "$version", "target_commitish":"${GIT_BRANCH}",  "body":  "$body"}
CURL_DATA
